...
def extract_function(func: str, fname: str='qaczar.py') -> str:
    ...
    lines, start, indent_level = [], 0, None
    for i, line in enumerate(source.splitlines()):
        if line.lstrip().startswith(f'def {func}('):
            start = i + 1; indent_level = len(line) - len(line.lstrip())
        elif start and len(line) - len(line.lstrip()) <= indent_level: break
        elif start: lines.append(line)
    return '\n'.join(lines)
...
def none(opt=None) -> None:
    if opt: return f'Alas {opt}' 
    pass
...
def watcher_role(*args, next: str = None, **kwargs) -> t.NoReturn:
    ...
    # TODO: This could later be used to gather the output of the process being watched.
